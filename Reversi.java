...
public class Reversi {
	public static void main(String[] args) {
		...
		Board b = new Board();//initialising a board
		...
		b.print(b);//printing board
		totalMoves(player,comp,b);
		while (true) {
			int counter = 0;//counter used in order to exit the game when both players have no valid moves
			//computer plays
			if (b.getLastLetterPlayed() == player  ) {
				if(!b.hasMoves(comp)){
					...
					Move WMove = computer.MiniMax(b); //using MiniMax method
					b.makeMove(WMove);//computer plays the move
					Move.print(WMove);
			...
			if (b.getLastLetterPlayed() == comp ) {
				if(!b.hasMoves(player)){
					...
					while (!Board.isValidMove(b.getGameBoard(), player, r, c)) { //getting user's input with check
				...
				if(counter ==2){//exit when both have no valid moves
                    b.setTerminal();
	...
	//getting user's input with check
	...
	private static int fromCharToInt(char c) { //converting char to int
		switch(c) {
		case 'a':
			return 0;
		case 'b':
	...
	//getting user's input for depth
    private static int getDepth() {
        ...
        return depth*8; //returning depth;
    }
